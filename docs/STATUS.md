...
| Viseme mapping | ✅ rule-based + phoneme dictionary + `MlVisemeMapper` with **trained weights** `assets/models/viseme_mlp.frvm` (TIMIT alignments, `fr_train_visemes`, 66.7 % speaker-disjoint acc vs 39.6 % baseline) | `.frvm` runs without LibTorch; TorchScript `.pt` needs `-DFR_WITH_TORCH=ON` |
...
| Tests | ✅ 31 Catch2 cases (32 with LibTorch) | FBX round-trip, trained mapper sanity, live pipeline on the test-signal device, OBJ parts / FRBS round-trip, ICT rig binding |
...
| UI | ✅ AccuRIG-style shell (`src/ui/theme.*`, `panels.cpp`): Inter font, dark/lime theme, left 5-step wizard incl. **Check Model orientation step**, right property page, export modal | Icons are glyph placeholders (no icon font vendored) |
