...
| Tests | ✅ 57 Catch2 cases (48 with LibTorch) | FBX round-trip, trained mapper sanity, live pipeline on the test-signal device, OBJ parts / FRBS round-trip, ICT rig binding |
...
| Graph editor | ✅ dockable window (`ui/graph_editor.cpp`, ImGui docking enabled): multi-channel key editing on the key layer, Normalised / Absolute / Stacked value modes, cross-channel box select + group move, tangent handles, numeric fields, `--graph-editor` demo, channel-view helpers in `ui/timeline_common.h` + test | Box-select picks by key position only (no marquee on curve segments) |
| Timeline editor | ✅ viewport dope-sheet (`panels.cpp` `timeline::`): ruler, **audio waveform + onsets**, word/phone/viseme lanes, paint mode with range ops, **bone-axis channels** (deg), **key layer** with Hermite tangents (Auto/Flat/Linear/Free, broken), kept across regeneration, flattened on export, serialised in clip JSON / projects box-select + group move, *Pose as keys* | Cross-channel dope-sheet rows not yet |
...
| Emotion from audio | ✅ `audio/emotion_classifier.cpp` + `tools/train_emotion.cpp`; CREMA-D 7442 clips, speaker-disjoint 59% acc / 58.9% UAR (6 classes); auto preset in generate + live 3 s window; UI/CLI `--emotion auto`; 4 tests | Fear/sad and happy/anger confusions remain (arousal-driven); speaker adaptation (item 20) adapts the viseme mapper only, not the emotion head |
| Speaker calibration | ✅ `audio/speaker_profile.cpp`: per-speaker feature mean/std (CMVN blended into the MLP input normalisation, strength 0.3), pitch median, loudness ref; optional last-layer fine-tune on the read calibration sentence (forced-aligned labels, Adam, anchored); profile JSON in `.frproj`, `--calibrate/--calibrate-text/--speaker/--save-speaker`; GUI Microphone tab records 4-10 s and re-arms the live mapper; `fr_eval_speaker` on TIMIT TEST (168 spk): 67.4 → 67.5 % stats, 90/168 speakers improve; 5 tests | Gains are small because the global model already normalises loudness/pitch; large strengths (≥0.5) hurt; a real mic recording was not available in the sandbox |
| Mic latency / VAD / AGC | ✅ `audio/mic_conditioner.cpp` (HPF, noise floor, VAD+hang gate, speech-only AGC) on the capture thread; latency budget + measured cadence/age/overruns; block/device-latency/smoothing controls; test-signal gaps + noise; 5 tests | Real-device tuning verified only on the synthetic path (no mic in the sandbox) |
| Mic self-test / report | ✅ `fr_cli --mic-check [report.md] [--mic-device i]`, `--mic-list` (`audio/mic_check.*`): silence / read-aloud / clap phases through the live path; noise floor, DC, 50/60 Hz hum, speech level, SNR, clipping, band balance, VAD share, viseme histogram, estimated + measured onset latency, overruns; graded OK/WARN/FAIL findings; writes .md + .json + .wav; 2 tests on the synthetic device | Needs to be run on a real machine - results feed back into conditioner defaults |
| Auto-landmarking | ✅ built-in SDM cascade trained on 101 ICT identities (`fr_train_landmarks`, 5.5 MB `assets/models/face_landmarks.frlm`, val NME 0.9 % inter-ocular on 10 held-out identities, nose-anchored box - no detector, no 99 MB dlib model); legacy dlib engine optional (`FR_WITH_DLIB`, `--landmarks dlib`) | Trained on ICT geometry: strongly non-human heads or heads seen far from frontal fall back to the proportional guess via the confidence gate |
| Auto-landmarking (legacy dlib) | ✅ dlib submodule (HOG detector + 68-pt predictor) on a software front render, lifted to the surface; drives default control points / shape centres; test on ICT; fallback to proportional guesses; UI mode + overlay, `--landmarks`, `--dump-landmarks` | Detector needs a shaded skin render - untextured smooth scans with no eye/mouth relief may not be found |
