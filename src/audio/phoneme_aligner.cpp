...
struct Unit { std::string phone; Viseme viseme; int wordIndex; bool optional; int minFrames; int typFrames; };
}
...
AlignmentResult PhonemeAligner::alignPhones(const std::vector<TranscriptWord>& words, const FeatureTrack& track, const std::vector<VisemeFrame>& posteriors) const {
    ...
    auto sil = [&](bool optional) { units.push_back({"SIL", Viseme::Silence, -1, optional, 1, typF}); };
    sil(true);
    for (size_t w = 0; w < words.size(); ++w) {
        for (const auto& ph : words[w].phones) {
            ...
            units.push_back({ph, v, int(w), false, minF, shortPh ? std::max(minF, typF / 2) : typF});
        }
    ...
    for (int u = 0; u < U; ++u) { prev[idx(u, 1)] = logP[0][size_t(units[size_t(u)].viseme)]; if (!units[size_t(u)].optional) break; }
    for (int t = 1; t < T; ++t) {
    ...
    int bestU = -1, bestD = -1; float best = kNegInf;
    for (int u = U - 1; u >= 0; --u) {
        for (int d = 1; d <= Dmax; ++d) { float s = prev[idx(u, d)]; if (s > best && d >= units[size_t(u)].minFrames) { best = s; bestU = u; bestD = d; } }
    ...
    for (const auto& ph : res.phones) {
        ...
        if (!res.segments.empty() && res.segments.back().viseme == ph.viseme) res.segments.back().end = ph.end;
        else res.segments.push_back({ph.viseme, ph.start, ph.end, 1.0f});
    }
